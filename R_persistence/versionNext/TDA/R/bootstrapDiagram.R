...
function(X, FUN, lim, by, sublevel=TRUE, library="Dionysus", B=30, alpha=0.05, distance="bottleneck", dimension=1, p=1, printProgress=FALSE, ...){
	...
	maxdimension=dimension
     ...
     Diag=gridDiag(X=X, FUN=FUN, lim=lim, by=by, maxdimension=maxdimension, sublevel=sublevel, library=library, printProgress=FALSE, diagLimit=NULL, ...)[["Diag"]]
     ...
     if (distance=="wasserstein") {
	     width=boostLapply(1:B, FUN=function(i){
	          ...
	          Y = as.matrix(X[I,])
	          Diag1 = gridDiag(X=Y, FUN=FUN, lim=lim, by=by, maxdimension=maxdimension, sublevel=sublevel, library=library, printProgress=FALSE, diagLimit=NULL, ...)[["Diag"]]
