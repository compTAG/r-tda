...
function(X, Grid, h){
	...
	Grid=as.matrix(Grid)
	out=rep(0, nrow(Grid))
	outR= .C("kdeDist", as.double(X), as.integer(nrow(X)), as.integer(ncol(X)), as.double(Grid), as.integer(nrow(Grid)), as.double(h), "out"=as.double(out),dup=FALSE, package="TDA")$out
	return(outR)
}
