plot.diagram <-
function(x, diagLim=NULL, dimension=NULL, col=NULL, rotated=FALSE, barcode=FALSE, band=NULL, colorBand=alpha(2,0.1), colorBorder=NULL, add=FALSE,...){
	...
	if (barcode){
		...
		if (rotated==TRUE){
			...
			if (!is.null(band)) 	polygon(c(0,diagLim[2]+1, diagLim[2]+1,0),c(0,0,band,band),col=colorBand, lwd=0.01, border=colorBorder)

			points((x[,2]+x[,3])/2, (x[,3]-x[,2])/2 ,col=col,pch=symb,lwd=2,cex=1)
		} else{

			if (add==FALSE) plot(0, 0,type="n", axes=F, xlim=diagLim, ylim=diagLim, xlab=" ", ylab=" ", ...)
			if (!is.null(band)) 	polygon(c(0,diagLim[2]+1,diagLim[2]+1,0),c(0,diagLim[2]+1, diagLim[2]+1+band,band),col=colorBand, lwd=0.01, border=colorBorder)
