...
void most_persistent_bars(double *diag_ptr, int max_num_bars) 
{
  ...
  for(auto pair : persistent_pairs_)
  {
    *diag_ptr = (double)cpx_->dimension(get<0>(pair)); ++diag_ptr;
    *diag_ptr = cpx_->filtration(get<0>(pair))       ; ++diag_ptr;
    *diag_ptr = cpx_->filtration(get<1>(pair))       ; ++diag_ptr;  
    ++count;
    if(count == max_num_bars) return;
  }
}
