...
void computePersistencePhat(Fltr f, const unsigned maxdimension,
	...
	for (typename Fltr::Index it = f.begin(); it != f.end(); ++it) {
		...
		simplex_map.insert(
				std::map< Smplx, phat::index >::value_type(c, size_of_simplex_map++));
	}
	...
	std::vector< unsigned > persLocPoint(2);
	unsigned persDim;

	// Manually add 0th homology for minimum to infinity
	persDgmPoint[0] = simplex_map_inv[0].data();
	...
	if (location) {
		...
		persLocPoint[0] = getLocation(simplex_map_inv[0], FUNvalues);
	...
	for (phat::index idx = 0; idx < pairs.get_num_pairs(); ++idx) {
		persDim = simplex_map_inv[pairs.get_pair(idx).first].dimension();
		persDgmPoint[0] = simplex_map_inv[pairs.get_pair(idx).first].data();
		persDgmPoint[1] = simplex_map_inv[pairs.get_pair(idx).second].data();
		if (persDgmPoint[0] < persDgmPoint[1] && persDim <= (unsigned)maxdimension) {
			persDgm[persDim].push_back(persDgmPoint);

			// trace back birth & death point
			if (location) {
				persLocPoint[0] = getLocation(
						simplex_map_inv[pairs.get_pair(idx).first], FUNvalues);
				persLocPoint[1] = getLocation(
						simplex_map_inv[pairs.get_pair(idx).second], FUNvalues);
