...
In Section \ref{sec:persistent}, we describe how to compute persistence diagrams of sublevel sets (or superlevel sets) of functions evaluated over a grid of points, using \code{gridDiag}, and how to compute the persistence diagram of the Rips filtration built on top of a point cloud, using \code{ripsDiag}. Our implementation relies on three \proglang{C++} libraries: \pkg{GUDHI} \citep{gudhi_stpcoh}, \pkg{Dionysus} \citep{dionysus_C}, and \pkg{PHAT} \cite{phat_C}.
...
The core of the function is written in \proglang{C++} and the user can choose to compute persistence diagram using either the \pkg{Dionysus} library or the \pkg{PHAT} library.
...
Diag = gridDiag( X=X, FUN=kde, h=0.3, lim=cbind(Xlim,Ylim), by=by, 
                 sublevel=F, library="Dionysus", printProgress=FALSE ) 
@
...
plot(Diag[["Diag"]], band=2*band$width, main="KDE Diagram")
@
...
plot(Diag[["Diag"]], rotated=TRUE, band=band$width, main="Rotated Diagram")
plot(Diag[["Diag"]], barcode=TRUE,  main="Barcode")
@
\end{center}
...
Diag1=ripsDiag(Circle1,maxdimension=1, maxscale=5, printProgress=F)
Diag2=ripsDiag(Circle2,maxdimension=1, maxscale=5, printProgress=F)
@
