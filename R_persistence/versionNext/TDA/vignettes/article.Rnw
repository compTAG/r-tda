...
\author{Brittany T. Fasy, Jisu Kim \and Fabrizio Lecci \\ \small{In collaboration with the CMU TopStat Group}}
\Plainauthor{Brittany Terese Fasy, Jisu Kim, Fabrizio Lecci}
...
The function \code{gridDiag} evaluates a given real valued function over a triangulated grid, constructs a filtration of simplices using the values of the function of dimension up to \code{maxDim+1}, and computes the persistent homology of the filtration of dimension up to \code{maxDim}.
...
Diag=ripsDiag(X=Circles, maxdimension, maxscale, printProgress=F)
@
...
maxKDE=maxPersistence(kde, parametersKDE, X, lim=cbind(Xlim, Ylim), 
                      by=by, sublevel=F, B=B, alpha=alpha, bandFUN="bootstrapBand",
