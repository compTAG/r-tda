...
for the efficient algorithms of the \proglang{C++} libraries \pkg{GUDHI}, \pkg{Dionysus} and \pkg{PHAT}, 
...
\proglang{C++} libraries \pkg{GUDHI} \citep{gudhi_stpcoh}, \pkg{Dionysus} \citep{dionysus_C}, and \pkg{PHAT} \citep{phat_C}.
...
The core of the function is written in \proglang{C++} and the user can choose to compute persistence diagrams using either the \pkg{Dionysus} library or the \pkg{PHAT} library.
...
The user can choose to compute the Rips persistence diagram using either the \proglang{C++} library \pkg{GUDHI}, or \pkg{Dionysus}.
...
The \code{alphaDiag} function computes the persistence diagram of the alpha shape filtration built on top of a point cloud in 3 dimension, using the C++ library \pkg{GUDHI}. Currently the point data cloud should be 3 dimension, and only work on 64-bit R.
...
and the following code compute the persistence diagram of the alpha shape filtration using the point cloud \code{XX}, with printing its progress. (\code{printProgress = FALSE}). The function \code{alphaDiag} returns an object of the class \code{"diagram"}.

<<eqAlpha2, echo=TRUE, eval=TRUE>>=
DiagAlpha <- alphaDiag(X = XX, printProgress = FALSE)
@
...
The \pkg{TDA} package includes the functions \code{bottleneck} and \code{wasserstein}, which are R wrappers of the functions ``bottleneck\_distance" and ``wasserstein\_distance" of the \proglang{C++} library \pkg{Dionysus}.
