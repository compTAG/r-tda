...
\author{Brittany T. Fasy, Jisu Kim, Cl\'ement Maria \and Fabrizio Lecci \\ \small{In collaboration with the CMU TopStat Group}}
\Plainauthor{Brittany Terese Fasy, Jisu Kim, Cl\'ement Maria, Fabrizio Lecci}
...
for the efficient algorithms of the \proglang{C++} libraries \pkg{GUDHI} and \pkg{Dionysus}, including a function for 
...
In Section \ref{sec:persistent}, we describe how to compute persistence diagrams of sublevel sets (or superlevel sets) of functions evaluated over a grid of points, using \code{gridDiag}, and how to compute the persistence diagram of the Rips filtration built on top of a point cloud, using \code{ripsDiag}. Our implementation relies on two \proglang{C++} libraries: \pkg{GUDHI}, developed by the Geometrica Group at INRIA and available at http://project.inria.fr/gudhi/software/; \pkg{Dionysus}, developed by Dmitriy Morozov and available at http://mrzv.org/software/dionysus/.
...
The core of the function is written in \proglang{C++} and uses the functionalities of the \pkg{Dionysus} library.
...
Diag = gridDiag( X=X, FUN=kde, h=0.3, lim=cbind(Xlim,Ylim), by=by, 
                 sublevel=FALSE, printProgress=FALSE ) 
@
...
plot(Diag, rotated=TRUE, band=band$width, main="Rotated Diagram")
...
Diag1=ripsDiag(Circle1,maxdimension=1, maxscale=5, printProgress=FALSE)
Diag2=ripsDiag(Circle2,maxdimension=1, maxscale=5, printProgress=FALSE)
@
...
PlotTriangles = function(left,right){
   ...
   plot(X,Y,type="n",xlab="",ylab="",xaxt="n",yaxt="n", xlim=xlimit, ylim=ylimit, frame.plot=FALSE, main="Triangles")
	axis(1)
	axis(2)
...
plot(tseq, Land1, col=4, type="l", lwd=2, xlim=xlimit, ylim=ylimit, axes=FALSE, main="1st Landscape", xlab="", ylab="")
axis(1)
axis(2)

plot(tseq, Sil1, col=4, type="l", lwd=2, xlim=xlimit, ylim=ylimit, axes=FALSE, main="Silhouette p=1", xlab="", ylab="")
axis(1)
axis(2)
@
\end{center}
...
maxKDE=maxPersistence(kde, parametersKDE, X, lim=cbind(Xlim, Ylim), 
                   by=by, sublevel=FALSE, B=B, alpha=alpha, parallel=TRUE, 
