...
The \code{alphaShapeDiag} function computes the persistence diagram of the alpha shape filtration built on top of a point cloud in 3 dimension, using the C++ library \href{ https://project.inria.fr/gudhi/software }{\pkg{GUDHI}}. Currently the point data cloud should lie in 3 dimension.
...
\begin{center}
<<eqAlphaShape3, echo=TRUE, eval=TRUE, fig=TRUE, height=2.8, width=2.8>>=
